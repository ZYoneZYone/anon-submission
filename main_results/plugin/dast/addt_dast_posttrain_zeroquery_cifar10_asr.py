...
STUDENT_WEIGHT_PATH = './baseline/DaST/sub_model_weight/resnet18_resnet34_cifar10.pth'
SYNTHETIC_DATASET_PATH = './baseline/DaST/generated_images' 
...
def resolve_paths(use_advtrained_target: bool):
    weight_suffix = "_advtrain" if use_advtrained_target else ""
    teacher_weight_path = TEACHER_WEIGHT_PATH
    student_weight_path = STUDENT_WEIGHT_PATH
    synthetic_dataset_path = SYNTHETIC_DATASET_PATH
    if use_advtrained_target:
        teacher_weight_path = os.path.join(
            "./target_model_weight/adv_trained",
            f"{TEACHER_MODEL_NAME}_{DATASET}_adv.pth",
        )
        student_weight_path = os.path.join(
            "./baseline/DaST/sub_model_weight",
            f"{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}{weight_suffix}.pth",
        )
        synthetic_dataset_path = SYNTHETIC_DATASET_PATH + weight_suffix
    return teacher_weight_path, student_weight_path, synthetic_dataset_path
...
def hard_pl_with_conf_weight(logits_t, logits_s,
    ...
    with torch.no_grad():
        ...
        probs_clone.scatter_(1, y_hat.unsqueeze(1), -1.0)
        p2, _ = probs_clone.max(dim=1)
        margin = pmax - p2
        ...
        B = logits_t.size(0)
        mask = torch.ones(B, dtype=torch.bool, device=logits_t.device)

    if not mask.any():
        zero = (logits_s * 0.0).sum()   # graph-connected zero
        return zero, 0.0, 0.0
...
def apply_on_batch(transform, x):
    """Apply torchvision v1 transform to a batch (B,C,H,W) by iterating per sample on CPU tensor.
    """
    return torch.stack([transform(img) for img in x], dim=0)


def compute_class_weights_from_loader(data_loader, num_classes: int):
    """Compute inverse-frequency class weights from synthetic pool.
    Returns a CPU tensor of shape [num_classes], normalized to mean=1.
    """
...
def zero_query_post_train(
    ...
    tau_high: float = 0.98,
    n_weak_views: int = 2,
    margin_th: float = 0.20,
    alpha_cons: float = 0.3,
    use_advtrained_target: bool = False,
    save_milestones=None,
):
    student = student.to(device).train()
    optimizer = torch.optim.SGD(student.parameters(), lr=lr, momentum=momentum)

    feat_dim = _get_classifier_fc(student).in_features
    ...
    best_asr = float('-inf')
    save_dir = "./zero_query_weight_asr"
    os.makedirs(save_dir, exist_ok=True)
    weight_suffix = "_advtrain" if use_advtrained_target else ""
    save_name = f"dast_addt_zeroquery_asr_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}{weight_suffix}.pth"
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            with torch.no_grad():
                ...
                logits_t = torch.stack(logits_list, dim=0).mean(0)   # [B,C]
            ...
            total_loss = loss_ce + alpha_cons * loss_cons + beta_supcon * loss_supcon
            ...
            epoch_supcon_loss_sum += loss_supcon.item()
        ...
        avg_supcon = epoch_supcon_loss_sum / num_batches if num_batches > 0 else 0.0

        acc, _ = test_acc(student, test_loader)
        con = test_cohen_kappa(student, teacher, test_loader)
        asr = test_robust_with_eval_params(student, teacher, test_loader, 'FGSM', False, 'cifar10')

        print("-" * 100)
        print(f"Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | Cons: {avg_cons:.4f} | SupCon: {avg_supcon:.4f}")
        ...
        if save_milestones:
            milestone = epoch + 1
            if milestone in save_milestones:
                base, ext = os.path.splitext(save_path)
                milestone_path = f"{base}_{milestone}{ext}"
                if os.path.isfile(save_path):
                    shutil.copy2(save_path, milestone_path)
                else:
                    torch.save(student.state_dict(), milestone_path)
                print(f"[Epoch {milestone}] Best-so-far checkpoint saved to {milestone_path}")


def load_teacher_model(teacher_weight_path: str, use_advtrained_target: bool = False):
    if use_advtrained_target:
        if not os.path.exists(teacher_weight_path):
            raise FileNotFoundError(f"Teacher (advtrained) weights not found at {teacher_weight_path}")
        teacher_model = get_advtrained_model(DATASET, TEACHER_MODEL_NAME)
        return teacher_model

    abs_weight_path = os.path.join(
        "./target_model_weight",
        f"{TEACHER_MODEL_NAME}_{DATASET}.pth",
    )
    if not os.path.exists(teacher_weight_path) and not os.path.exists(abs_weight_path):
        raise FileNotFoundError(
            f"Teacher weights not found at {teacher_weight_path} or {abs_weight_path}"
        )
    teacher_model = get_target_model(DATASET, TEACHER_MODEL_NAME)
    return teacher_model


def load_student_model(student_weight_path: str):
...
def load_synthetic_dataset_and_loader(root: str):
    dataset = DATASET
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Post-train zero-query with ASR-based model selection")
    parser.add_argument(
        "--use_advtrained_target",
        action="store_true",
        help="Use adversarially trained target model as teacher",
    )
    parser.add_argument("--epochs", type=int, default=EPOCHS, help="Post-train epochs")
    parser.add_argument("--tau_low", type=float, default=0.88, help="Lower confidence threshold for pseudo-label weighting")
    parser.add_argument("--tau_high", type=float, default=0.985, help="Upper confidence threshold for pseudo-label weighting")
    parser.add_argument("--n_weak_views", type=int, default=2, help="Number of weak views for teacher ensembling")
    parser.add_argument("--margin_th", type=float, default=0.12, help="Top1-Top2 margin threshold for selecting pseudo-labels")
    parser.add_argument("--alpha_cons", type=float, default=0.25, help="Weight for consistency loss term")
    return parser
...
def main():
    ...
    teacher_weight_path, student_weight_path, synthetic_dataset_path = resolve_paths(args.use_advtrained_target)

    setup_seed(2021)
    _, test_loader = get_dataset(DATASET)

    teacher = load_teacher_model(teacher_weight_path, use_advtrained_target=args.use_advtrained_target)
    student = load_student_model(student_weight_path)
    ...
    syn_dataset, syn_loader = load_synthetic_dataset_and_loader(synthetic_dataset_path)
    ...
    print("=" * 100)
    print("TRAINING WITH ASR-BASED MODEL SELECTION")
    print("Model will be saved based on best ASR (FGSM, eps=8.0/255.0)")
    print("=" * 100)
    ...
    zero_query_post_train(
        ...
        use_advtrained_target=args.use_advtrained_target,
        save_milestones=([30, 50, 80, 100] if args.epochs == 100 else None),
    )
