...
def hard_pl_with_conf_weight(logits_t, logits_s,
    ...
    with torch.no_grad():
        ...
        if top_frac is not None and mask.any():
            kth = torch.quantile(pmax[mask], 1.0 - top_frac)
            mask = mask & (pmax >= kth)
...
def compute_class_weights_from_loader(data_loader, num_classes: int):
    """Compute inverse-frequency class weights from the synthetic pool.
...
def zero_query_post_train(
    ...
    # [Multi-Task Init]
    fc_main = _get_classifier_fc(student)
    fc_aux = copy.deepcopy(fc_main).to(device).train()
    
    params = list(student.parameters()) + list(fc_aux.parameters())
    optimizer = torch.optim.SGD(params, lr=lr, momentum=momentum)

    feat_dim = fc_main.in_features
    # bank = TinyMemoryBank(feat_dim, NUMBER_CLASSES, max_size=8192, device=device)
    ...
    best_asr = float('-inf')
    save_dir = "./ablation_results/gradient_multitask"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"gradient_multitask_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    save_path = os.path.join(save_dir, save_name)

    print(f"Start Ablation Training: Multi-Task: Auxiliary Classifier Head on {DATASET}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            # Student on strong (unmixed)
            # [Multi-Task Forward]
            logits_s_main, feats_strong = grab_pre_fc_feats_once(student, images_s)
            
            logits_s_aux = fc_aux(feats_strong)

            # [Consistency Loss] 
            loss_cons, cons_used_frac, cons_avg_w = hard_pl_with_conf_weight(
                logits_t,
                logits_s_aux,  # <--- Aux Head
                tau_low=tau_low,
                tau_high=tau_high,
                T=T_SHARP,
                margin_th=margin_th,
            ...
            # CE: apply MixUp/CutMix only for CE branch on strong view
            if use_mixup:
                ...
                # MixUp forward: Main head only
                logits_mix = student(images_mix)
                loss_ce_mix = soft_ce(logits_mix, y_soft)
            else:
                batch_size = labels.size(0)
                loss_ce_mix = F.cross_entropy(logits_s_main, labels, reduction="sum") / batch_size

            ce_hard_balanced = F.cross_entropy(logits_s_main, labels, weight=class_weights)
...
def load_teacher_model():
    # Use original method to load teacher (includes loading weights internally)
    # Path check only for sanity with provided absolute path
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Gradient Decoupling Ablation: Multi-Task")
    # [Control Variates] FMNIST Specific Params preserved
