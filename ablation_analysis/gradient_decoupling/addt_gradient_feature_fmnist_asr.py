...
# ==========================================
#      Gradient Decoupling: Feature-Level Consistency
# ==========================================
...
def feature_mse_with_conf_weight(feats_t, feats_s, logits_t,
    ...
    with torch.no_grad():
        ...
        mask = torch.ones(feats_s.size(0), dtype=torch.bool, device=feats_s.device)

    mse_i = F.mse_loss(feats_s[mask], feats_t[mask], reduction='none').mean(dim=1)
    
    loss = (mse_i * w[mask]).sum() / (w[mask].sum() + 1e-8)
...
def zero_query_post_train(
    ...
    best_asr = float('-inf')
    save_dir = "./ablation_results/gradient_feature"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"gradient_feature_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    save_path = os.path.join(save_dir, save_name)

    print(f"Start Ablation Training: Feature-Level: MSE Consistency on Pre-Classifier Features on {DATASET}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            with torch.no_grad():
                ...
                for _ in range(n_weak_views):
                    ...
                    # Apply hook on the underlying teacher model
                    l, f = grab_pre_fc_feats_once(ema_teacher.teacher, images_w_i)
                    logits_list.append(l)
                    feats_list.append(f)
                
                logits_t = torch.stack(logits_list, dim=0).mean(0) # [B, C]
                feats_t = torch.stack(feats_list, dim=0).mean(0)   # [B, D]

            logits_s_strong, feats_strong = grab_pre_fc_feats_once(student, images_s)

            loss_cons, cons_used_frac, cons_avg_w = feature_mse_with_conf_weight(
                feats_t,
                feats_strong, 
                tau_low=tau_low,
                tau_high=tau_high,
            ...
            # ====================================================
            # [ABLATION CORE: Feature Consistency]
            # ====================================================
            total_loss.backward()
        ...
        print("-" * 120)
        print(f"Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | FeatMSE: {avg_cons:.4f} ")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Gradient Decoupling Ablation: Feature-Level")
    # [Control Variates] FMNIST Specific Params
