...
# ==========================================
#        Gradient Decoupling: Mixed (no classifier-head blocking)
# ==========================================
...
def zero_query_post_train(
    ...
    save_dir = "./ablation_results/gradient_mixed"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"gradient_mixed_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    save_path = os.path.join(save_dir, save_name)

    print(f"Start Ablation Training: Mixed: No Classifier-Head Blocking on {DATASET}")
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            # Student on strong (unmixed) for Consistency + SupCon
            ...
            # ====================================================
            # [ABLATION CORE: Mixed Strategy]
            # ====================================================
            if cons_used_frac > 0:               
                loss_graph.backward(retain_graph=True)
                # ❌ [Removed] if fc.weight.grad is not None: fc.weight.grad.zero_()
                # ❌ [Removed] if fc.bias.grad is not None: fc.bias.grad.zero_()
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Gradient Decoupling Ablation: Mixed")
