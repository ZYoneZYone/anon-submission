...
def zero_query_post_train(
    ...
    # ========================================
    # ========================================
    #   weak = T.Compose([T.RandomCrop(28, padding=2)])
    #   strong = T.Compose([T.RandomAffine(...)])
    # 
    # ========================================
    ...
    best_asr = float('-inf')
    save_dir = "./ablation_results/multiview_1+3"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"multiview_1+3_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            # ========================================
            # ========================================
            images_orig = images.to(device).float()
            ...
            # ========================================
            with torch.no_grad():
                logits_t = ema_teacher(images_orig)

            # ========================================
            # ========================================
            logits_s_orig, feats_orig = grab_pre_fc_feats_once(student, images_orig)
            ...
            if use_mixup:
                if np.random.rand() < 0.5:
                    images_mix, y_soft = mixup_batch(images_orig, labels, alpha=0.4)
                else:
                    images_mix, y_soft = cutmix_batch(images_orig, labels, alpha=1.0)
                logits_mix = student(images_mix)
                loss_ce_mix = soft_ce(logits_mix, y_soft)
            else:
                batch_size = labels.size(0)
                loss_ce_mix = F.cross_entropy(logits_s_orig, labels, reduction="sum") / batch_size

            ce_hard_balanced = F.cross_entropy(logits_s_orig, labels, weight=class_weights)
        ...
        print("-" * 120)
        print(f"[Multi-View EMA: DGDT + Mix-aug + EMA] Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | Cons: {avg_cons:.4f}")
        ...
        print(f"Cons-used: {cons_used_frac:.3f} (should be 1.000) | Cons-avgW: {cons_avg_w:.3f}")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Ablation: w/o Dual-Gate + w/o Strong-Weak Views with ASR-based selection")
    ...
    # parser.add_argument("--alpha_cons", type=float, default=0.4, help="Weight for consistency loss term")
...
def main():
    ...
    print("=" * 120)
    print("ABLATION STUDY: w/o Dual-Gate + w/o Strong-Weak Views with ASR-based selection")
    print("=" * 120)
    print("Double Ablation - Removed TWO components:")
    print("  1. Dual-Gate: Remove confidence+margin gating - use ALL samples (mask = torch.ones)")
    print("  2. Strong-Weak Views: Remove augmentation - both teacher and student use ORIGINAL images")
    print("")
    print("Preserved mechanisms:")
    print("  ✓ Gradient Decoupling (fc.grad.zero_() after consistency backward)")
    print("  ✓ EMA Teacher (temporal smoothing)")
    print("  ✓ Confidence Weighting (soft weighting based on tau_low/tau_high)")
    print("")
    print("Model selection: Based on best ASR (FGSM, eps=8.0/255.0 for MNIST)")
    print("=" * 120)
