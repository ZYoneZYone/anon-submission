...
def zero_query_post_train(
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            #   images_w = apply_on_batch(weak, images).to(device).float()
            # 
        ...
        print("-" * 120)
        print(f"[w/o Strong-Weak Views] Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | Cons: {avg_cons:.4f}")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Ablation: w/o Strong-Weak Views with ASR-based selection")
    ...
    parser.add_argument("--n_weak_views", type=int, default=3, help="Number of weak views (NOT USED in this ablation)")
...
def main():
    ...
    print("=" * 120)
    print("ABLATION STUDY: w/o Strong-Weak Views with ASR-based selection")
    print("=" * 120)
    print("Modification: Remove strong-weak augmentation views")
    print("  - Teacher: Uses ORIGINAL images (no weak augmentation, no multi-view ensembling)")
    print("  - Student: Uses ORIGINAL images (no strong augmentation)")
    print("  - Both teacher and student see the SAME unaugmented images")
    print("Model selection: Based on best ASR (FGSM, eps=0.3 for FMNIST)")
    print("Preserved mechanisms: Gradient Decoupling + EMA Teacher + Confidence Weighting")
    print("=" * 120)
