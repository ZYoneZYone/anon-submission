...
def hard_pl_with_conf_weight(logits_t, logits_s,
    ...
    with torch.no_grad():
        ...
        # ========================================
        # ========================================
        # mask = (pmax >= tau_low) & (margin >= margin_th)
...
def zero_query_post_train(
    ...
    best_asr = float('-inf')
    save_dir = "./ablation_results/multiview_1+2"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"multiview_1+2_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            with torch.no_grad():
                ...
                for _ in range(n_weak_views):
                    images_w_i = apply_on_batch(weak, images).to(device).float()
                    logits_list.append(student(images_w_i))
                logits_t = torch.stack(logits_list, dim=0).mean(0)
            # ========================================

            # Student on strong (unmixed) for Consistency
        ...
        print("-" * 120)
        print(f"[Multi-View SWV: DGDT + Mix-aug + SWV] Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | Cons: {avg_cons:.4f}")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Multi-View Decomposition (1+2): DGDT + Mix-augmented + SWV")
...
def main():
    ...
    print("=" * 120)
    print("ABLATION STUDY: w/o EMA + w/o Dual-Gate with ASR-based selection")
    print("Modification 1: Remove EMA mechanism - use student's current predictions as pseudo-labels")
    print("Modification 2: Remove Dual-Gate mechanism - use ALL samples (mask = torch.ones)")
    print("Model selection: Based on best ASR (FGSM, eps=0.3)")
    print("=" * 120)
