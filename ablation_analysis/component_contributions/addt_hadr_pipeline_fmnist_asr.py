...
def zero_query_post_train(
    ...
    best_asr = float('-inf')
    save_dir = "./ablation_results/hadr_pipeline"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"hadr_pipeline_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (images, labels) in enumerate(syn_loader):
            ...
            #   loss_ce.backward()
            #
            total_loss = alpha_cons * loss_cons + loss_ce
            total_loss.backward()
            # ========================================
        ...
        print("-" * 120)
        print(f"[HADR Pipeline: Joint Training] Epoch {epoch+1}/{epochs} - Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | Cons: {avg_cons:.4f}")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Ablation: w/o Dual-Gate + w/o Gradient Decoupling with ASR-based selection")
    parser.add_argument("--tau_low", type=float, default=0.75, help="Lower confidence threshold (for weight calculation only)")
    parser.add_argument("--tau_high", type=float, default=0.93, help="Upper confidence threshold (for weight calculation only)")
...
def main():
    ...
    print("=" * 120)
    print("ABLATION STUDY: HADR Data Refinement with Joint Training")
    print("=" * 120)
    print("HADR data refinement pipeline: EMA teacher + multi-view consistency + MixUp/CutMix + class-balanced CE")
    print("Training uses standard joint optimization without gradient decoupling")
    print("")
