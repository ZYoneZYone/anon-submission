...
def zero_query_post_train(
    ...
    best_asr = float('-inf')
    save_dir = "./ablation_results/dgdt_pipeline"
    os.makedirs(save_dir, exist_ok=True)
    save_name = f"dgdt_pipeline_{STUDENT_MODEL_NAME}_{TEACHER_MODEL_NAME}_{DATASET}.pth"
    ...
    for epoch in range(epochs):
        ...
        for _, (images, labels) in enumerate(syn_loader):
            ...
            with torch.no_grad():
                logits_t = student(images_orig)

            logits_s_orig, _ = grab_pre_fc_feats_once(student, images_orig)
        ...
        print("-" * 120)
        print(
            f"[DGDT Pipeline: Decoupled Gradient Training] Epoch {epoch+1}/{epochs} - "
            f"Loss: {avg_loss:.4f} | CE: {avg_loss_ce:.4f} | Cons: {avg_cons:.4f}"
        )
        print(f"Eval - Acc: {acc:.2f}% | Cohen's Kappa: {con:.2f}% | ASR: {asr:.2f}%")
        cons_used_frac_ep = epoch_cons_used_frac_sum / num_batches if num_batches > 0 else 0.0
        cons_avg_w_ep = epoch_cons_avg_w_sum / num_batches if num_batches > 0 else 0.0
        print(f"Cons-used: {cons_used_frac_ep:.3f} | Cons-avgW: {cons_avg_w_ep:.3f}")
...
def build_arg_parser():
    parser = argparse.ArgumentParser(description="Ablation: w/o EMA + w/o Strong-Weak with ASR-based selection")
...
def main():
    ...
    print("=" * 120)
    print("ABLATION STUDY: DGDT Gradient Decoupling with Single-View Input")
    print("DGDT gradient decoupling pipeline: Stage-1/Stage-2 gradient isolation with single-view input, without EMA teacher, multi-view consistency, or class-balanced CE")
    print("=" * 120)
